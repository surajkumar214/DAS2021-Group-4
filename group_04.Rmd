...
#read and stored the data in dataset4, remove the Region variable 
...
dataset4 <- dataset4 %>%
    slice(-c(2033))
# fitted the model again everything being as earlier 
model1 <- glm(Total.Number.of.Family.members ~ 
                Household.Head.Sex                                                   +
...
```{r drop1}
# drop1 perform F test, gives the cost of removing each variable by comparing deviance 
...
kable_styling(latex_options = 'HOLD_position') # to hold the middle position
...
```{r mod4}
# new model with removing another insignifact interaction term
