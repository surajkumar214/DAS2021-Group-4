...
```
The F test says that we can remove log(Household.Head.Age):Type.of.Household and Type.of.Household:log(Total.Food.Expenditure). So, repeat the same process until we reach all terms are significant. 
...
```
Eventually, we reached to our final model that considers interactions between  log(Total.Household.Income) and Electricity as significant. Now, we adjust the standard error using dispersion parameter, which is equivalent of fitting a quasipoission model.  


```{r final}
X2 <- sum(resid(model4, type = "pearson")^2)
dp <- X2 / model4$df.res
dp
model4 %>%
 summ( dispersion =dp)
...
```
We can see that standard error slight rises up after adjusting with the dispersion parameter, while the intercept term remains the same. Still , the wald test is not very reliable to draw inferences and may conflict with the F test. 
